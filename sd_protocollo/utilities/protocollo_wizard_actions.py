...
class ProtocolloWizardActions(models.Model):
    ...
    def protocollo_registra_action(self):
        ...
        if html_errors:
            return self.env.user._request_notify_message(
                "danger", "Registrazione Protocollo", html_errors
            )
        return {
    ...
    def protocollo_aggiungi_mittente_interno_action(self):
        ...
        ufficio_data_list = self.env["fl.set.voce.organigramma"].search_read(
            [("tipologia", "=", "ufficio")], ["id"]
        )
        mittente_disable_ids = [
            ufficio_data["id"] for ufficio_data in ufficio_data_list
        ]
        context = self._get_context_aggiungi_mittente_interno_action(
            mittente_disable_ids
        )
        return {
            "name": context.get("wizard_label", ""),
    ...
    def protocollo_assegna_action(self):
        ...
        prima_assegnazione = self.is_prima_assegnazione(
            self.env.context.get("modifica_assegnatari", False)
        )
        disable_ids = organigramma_obj.get_disable_ids(prima_assegnazione)
        assegnatario_disable_dictionary_ids = (
            self.get_assegnatario_disable_dictionary_ids("assegna")
        )
        assegnatario_competenza_disable_ids = (
            disable_ids + assegnatario_disable_dictionary_ids["competenza"]
        )
        assegnatario_conoscenza_disable_ids = (
            disable_ids + assegnatario_disable_dictionary_ids["conoscenza"]
        )
        context = dict(
            self.env.context,
            protocollo_id=self.id,
    ...
    def protocollo_assegna_conoscenza_action(self):
        ...
        disable_ids = organigramma_obj.get_disable_ids(prima_assegnazione)
        assegnatario_disable_dictionary_ids = (
            self.get_assegnatario_disable_dictionary_ids(
                "assegna_conoscenza", "conoscenza"
            )
        )
        assegnatario_conoscenza_disable_ids = (
            disable_ids + assegnatario_disable_dictionary_ids["conoscenza"]
    ...
    def protocollo_prendi_in_carico_assegnazione_action(self):
        self.ensure_one()
        assegnazione_competenza_ids = self.get_assegnazione_ids(
            "competenza", "assegnato", self.env.uid
        )
        if len(assegnazione_competenza_ids) == 0:
            ...
            errore = _(
                "L'utente ha più di una assegnazione per competenza nel protocollo!"
            )
            raise ValidationError(errore)
        esito, errore = self.prendi_in_carico_assegnazione(
            assegnazione_competenza_ids[0], self.env.uid
        )
    ...
    def protocollo_riassegna_action(self):
        ...
        assegnazione_rifiutata_ids = self.get_assegnazione_assegnatore_ids(
            "competenza", "rifiutato", self.env.uid
        )
        assegnatario_disable_dictionary_ids = (
            self.get_assegnatario_disable_dictionary_ids(
                "riassegnazione",
    ...
    def protocollo_leggi_assegnazione_action(self):
        ...
        if len(assegnazione_da_leggere_ids) == 0:
            ...
            esito, errore = self.leggi_assegnazione(
                assegnazione_da_leggere_ids[0], self.env.uid
            )
            if not esito and errore:
                raise ValidationError(errore)
        else:
    ...
    def protocollo_completa_lavorazione_action(self):
        ...
        visualizza_motivazione = bool(
            config_obj.get_param(
                "sd_protocollo.visualizza_motivazione_completa_lavorazione"
            )
        )
        error_protocol_list = []
        for protocollo in self:
            is_protocollatore = (
                protocollo.protocollatore_id.id == self.env.uid
                and protocollo.protocollatore_stato == "lavorazione"
            )
            assegnazione_ids = (
                protocollo.get_assegnazione_da_completare_lavorazione_ids(self.env.uid)
            )
            if not is_protocollatore and len(assegnazione_ids) == 0:
                ...
                raise ValidationError(errore)
            if (
                not visualizza_motivazione
                and is_protocollatore
                and len(assegnazione_ids) == 0
            ):
                protocollo.completa_lavorazione_protocollatore(self.env.uid, None)
                continue
            if (
                not visualizza_motivazione
                and not is_protocollatore
                and len(assegnazione_ids) == 1
            ):
                protocollo.completa_lavorazione_assegnazione(
                    assegnazione_ids[0], self.env.uid, None
                )
                continue
            if protocollo_count == 1:
                context = dict(
        ...
        if error_protocol_list:
            raise ValidationError(
                "Nei seguenti protocolli non è stato possibile completare la procedura di completamento della lavorazione:\n%s"
    ...
    def protocollo_rimetti_in_lavorazione_action(self):
        ...
        is_protocollatore = (
            ...
            and self.protocollatore_stato == "lavorazione_completata"
        )
        assegnazione_ids = self.get_assegnazione_da_rimettere_in_lavorazione_ids(
            self.env.uid
        )
        if not is_protocollatore and len(assegnazione_ids) == 0:
        ...
        if not is_protocollatore and len(assegnazione_ids) == 1:
            return self.rimetti_in_lavorazione_assegnazione(
                assegnazione_ids[0], self.env.uid, None
            )
        context = dict(
            self.env.context,
    ...
    def protocollo_invia_action(self):
        ...
        if self.mezzo_trasmissione_id:
            invio_obj.crea_invio_analogico(
                self.id, self.mezzo_trasmissione_id.id, self.destinatario_ids.ids
            )
            self.env.user._request_notify_message(
                "success", "Invio Analogico", "Invio creato con successo"
            )
