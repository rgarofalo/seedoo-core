...
class RegistroGiornaliero(models.Model):
    ...
    nome = fields.Char(
        string="Nome", compute="_compute_nome", store=True, readonly=True
    )
    ...
    data_chiusura = fields.Datetime(string="Data ed Ora Chiusura", readonly=True)

    protocollo_id = fields.Many2one(
        string="Protocollo", comodel_name="sd.protocollo.protocollo", readonly=True
    )
    ...
    documento_id = fields.Many2one(
        string="Documento", comodel_name="sd.dms.document", readonly=True
    )

    documento_id_content = fields.Binary(
        string="File", related="documento_id.content", readonly=True
    )

    documento_id_filename = fields.Char(
        string="Nome File", related="documento_id.filename", readonly=True
    )

    field_readonly = fields.Boolean(
        string="Field Invisible", compute="compute_field_readonly", readonly=True
    )
    ...
    def render_pdf(self, registro_giornaliero):
        report_registro_giornaliero = self.env.ref(
            "sd_protocollo.report_sd_protocollo_registro_giornaliero_qweb"
        )
        registro_giornaliero_data = report_registro_giornaliero.sudo()._render_qweb_pdf(
    ...
    def action_chiudi(self):
        ...
        for rec in self:
            folder = self.env["sd.protocollo.cartella"].get_folder(
                rec, "registro_giornaliero"
            )
            if not folder:
                raise ValidationError(
                    "La cartella per il salvataggio dei registri giornalieri non è configurata"
                )
            ...
            documento = documento_obj.create(
                {
                    ...
                    "document_type_id": self.env.ref(
                        "sd_protocollo.data_sd_dms_document_type_rgp"
                    ).id,
                    "producer": "Metafora",
                }
            )
    ...
    def genera_registri_mancanti(self):
        ...
        for company in company_list:
            if (
                registro_obj.search_count([("ufficiale", "=", True), ("company_id", "=", company.id)]) == 0
            ):
                continue
    ...
    def crea_registro_giornaliero(self, **kwargs):
        ...
        if registro_giornaliero:
            ...
            registro_giornaliero_data = datetime.datetime.strptime(
                str(registro_giornaliero.data), DEFAULT_SERVER_DATE_FORMAT
            )
        ...
        for date_day in date_list:
            try:
                _logger.debug(
                    "Creazione registro giornaliero con valori: %s" % str(kwargs)
                )
    ...
    def protocolla_registro_giornaliero(self, documento):
        self.ensure_one()
        registro_configurazione_obj = self.env[
            "sd.protocollo.registro.giornaliero.configurazione"
        ]
        protocollo_obj = self.env["sd.protocollo.protocollo"]
        contact_obj = self.env["sd.dms.contact"]

        config = registro_configurazione_obj.search(
            self._get_domain_configurazione_registro_giornaliero()
        )
        if not config:
            return
        ...
        try:
            protocollo = protocollo_obj.with_context(
                cron_company_id=config.company_id.id
            ).create(protocollo_vals)
            protocollo.registra()
    ...
    def create(self, vals):
        ...
        selected_start_date = datetime.datetime(
            rec.data.year, rec.data.month, rec.data.day
        )
    ...
    def action_show_protocollo_ids(self):
        ...
        return {
            ...
            "target": "current",
            "context": {"search_default_registro_giornaliero_id": self.id},
        }
