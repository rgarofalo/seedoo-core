...
class ResPartner(models.Model):
    ...
    company_subject_type = fields.Selection(
        string="Company typology", selection=SELECTION_COMPANY_SUBJECT_TYPE
    )

    contact_pa_type = fields.Selection(
        string="Company PA typology", selection=SELECTION_CONTACT_PA_TYPE
    )
    ...
    display_name_html = fields.Html(
        string="Display name html", compute="_compute_display_name", store=True
    )
    ...
    is_private_company = fields.Boolean(
        string="Is a private company", compute="_compute_company_values", store=True
    )

    is_pa = fields.Boolean(
        string="Is a PA/GPS", compute="_compute_company_values", store=True
    )

    is_amm = fields.Boolean(
        string="Is a PA", compute="_compute_company_values", store=True
    )

    is_gps = fields.Boolean(
        string="Is a GPS", compute="_compute_company_values", store=True
    )

    is_aoo = fields.Boolean(
        string="Is a AOO", compute="_compute_company_values", store=True
    )

    is_uo = fields.Boolean(
        string="Is a UO", compute="_compute_company_values", store=True
    )

    # Amministrazione

    amministrazione_id = fields.Many2one(string="PA / GPS", comodel_name="res.partner")

    cod_amm = fields.Char(string="IPA code")
    ...
    cod_aoo = fields.Char(string="AOO code")
    ...
    cod_ou = fields.Char(string="UO code")

    parent_amm_name = fields.Char(string="Parent PA name", compute="_compute_parent_pa")

    parent_gps_name = fields.Char(
        string="Parent GPS name", compute="_compute_parent_pa"
    )

    parent_aoo_name = fields.Char(
        string="Parent AOO name", compute="_compute_parent_pa"
    )

    parent_uo_name = fields.Char(string="Parent UO name", compute="_compute_parent_pa")

    child_ids = fields.One2many(compute="_compute_child_ids", readonly=False)
    ...
    def _get_domain_aoo_id(self):
        ...
        if self.env.context.get("from_pa", False):
            domain.append(
                ("amministrazione_id", "=", self.env.context.get("amministrazione_id"))
            )
        return domain
    ...
    def _compute_parent_id(self):
        for rec in self:
            ...
            if rec.parent_id and rec.parent_id.is_aoo:
                ...
                rec.parent_child_aoo_id = (
                    rec.parent_id.aoo_id.id if rec.parent_id.aoo_id else False
                )
    ...
    def _inverse_parent_child_id(self):
        for rec in self:
            ...
            elif rec.parent_id and (
                rec.parent_id.is_uo or rec.parent_id.is_aoo or rec.parent_id.is_amm
            ):
    ...
    def _compute_company_values(self):
        for rec in self:
            rec.is_private_company = (
                rec.company_subject_type == "private" and rec.contact_pa_type != "uo"
            )
            rec.is_pa = rec.company_subject_type in ["pa", "gps"]
            rec.is_amm = rec.company_subject_type == "pa" and not rec.contact_pa_type
            rec.is_gps = rec.company_subject_typ == "gps" and not rec.contact_pa_type
            rec.is_aoo = (
                rec.company_subject_type in ["pa", "gps"]
                and rec.contact_pa_type == "aoo"
            )
            rec.is_uo = (
                rec.company_subject_type in ["private", "pa", "gps"]
                and rec.contact_pa_type == "uo"
            )
    ...
    def _compute_pa_digital_domicile_ids(self):
        ...
        for rec in self:
            if not rec.is_amm and not rec.is_gps:
                ...
                return
            digital_domicile_ids = digital_domicile_obj.search(
                [("aoo_id", "in", rec.aoo_ids.ids)]
            ).ids
    ...
    def _compute_uo_digital_domicile_ids(self):
        ...
        for rec in self:
            ...
            digital_domicile_ids = (
                digital_domicile_obj.search([("aoo_id", "in", [rec.aoo_id.id])]).ids
                if rec.aoo_id
                else []
            )
    ...
    @api.depends(
        ...
        "company_name",
        "amministrazione_id",
        "amministrazione_id.name",
        "amministrazione_id.display_name",
        "aoo_id",
        "aoo_id.display_name",
        "aoo_id.name",
    )
        ...
        for partner in self:
            ...
            if partner.is_aoo or partner.is_amm or partner.is_gps:
                ...
                if partner.parent_id.is_uo and partner.parent_id.name:
                    ...
                    partner.parent_id.is_amm or partner.parent_id.is_gps
    ...
    def _get_name(self):
        ...
        if (
            self.amministrazione_id
            or self.aoo_id
            or (
                self.parent_id
                and (
                    self.parent_id.is_aoo
                    or self.parent_id.is_uo
                    or self.parent_id.is_amm
                    or self.parent_id.is_gps
                )
            )
        ):
    ...
    def _onchange_company_type(self):
        ...
        if (
            ...
            or self.company_subject_type != "pa"
            or self.company_subject_type != "gps"
        ):
    ...
    def _compute_parent_pa(self):
        for rec in self:
            ...
            if r.is_amm:
                ...
                rec.parent_amm_name = (
                    r.amministrazione_id.name if r.amministrazione_id.is_amm else False
                )
                rec.parent_gps_name = (
                    r.amministrazione_id.name if r.amministrazione_id.is_gps else False
                )
                rec.parent_aoo_name = r.name
            elif r.is_uo:
                rec.parent_amm_name = (
                    r.aoo_id.amministrazione_id.name
                    if r.aoo_id.amministrazione_id.is_amm
                    else False
                )
                rec.parent_gps_name = (
                    r.aoo_id.amministrazione_id.name
                    if r.aoo_id.amministrazione_id.is_gps
                    else False
                )
    ...
    def onchange_parent_child_aoo_id(self):
        ...
        caso2 = (
            self.parent_child_aoo_id
            and self.parent_child_uo_id
            and self.parent_child_uo_id.aoo_id
            and self.parent_child_uo_id.aoo_id.id != self.parent_child_aoo_id.id
        )
        if caso1 or caso2:
            self.parent_child_uo_id = False
        ...
        for rec in self:
            ...
            if rec.aoo_ids:
                raise ValidationError(
                    _("It is not possible to delete a record with AOO!")
                )
        super(ResPartner, self).unlink()

    def write(self, vals):
        for rec in self:
            is_company_type_updated = (
                vals.get("company_type", False)
                and rec.company_type
                and rec.company_type != vals.get("company_type", False)
            )
            is_company_subject_type_updated = (
                vals.get("company_subject_type", False)
                and rec.company_subject_type
                and rec.company_subject_type != vals.get("company_subject_type", False)
            )
            if (
                is_company_type_updated or is_company_subject_type_updated
            ) and rec.aoo_ids:
                raise ValidationError(
                    _(
                        "It is not possible to change the type of contact with associated AOO!"
                    )
                )
            ...
            if "parent_child_uo_id" in vals and not ("parent_child_aoo_id" in vals):
                vals["parent_child_aoo_id"] = (
                    rec.parent_child_aoo_id.id if rec.parent_child_aoo_id else False
                )
            ...
            if "child_ids" in vals and vals["child_ids"]:
                self._create_or_unlink_computed_one2many(
                    vals["child_ids"], "res.partner", "parent_id", rec.id
                )
        return super(ResPartner, self).write(vals)

    @api.model
    def _create_or_unlink_computed_one2many(
        self, data_list, many2one_model, many2one_field=None, many2one_id=None
    ):
    ...
    def get_uniqueness_mail_models(self):
        ...
        uniqueness_mail_models += [
            "res.partner.email.address",
            "res.partner.digital.domicile",
        ]
        return uniqueness_mail_models

    @api.model
    def get_uniqueness_mail_domain(
        self, mail_type, value, model_name, check_id, parent_id=False
    ):
        domain = super(ResPartner, self).get_uniqueness_mail_domain(
            mail_type, value, model_name, check_id, parent_id
        )
        ...
        if (
            ...
            and model_name == "res.partner.digital.domicile"
            and parent_id
        ):
