...
class Mail(models.Model):
    ...
    protocollo_id = fields.Many2one(
        string="Protocol", comodel_name="sd.protocollo.protocollo"
    )

    protocollo_in_id = fields.Many2one(
        string="Protocol from mail", comodel_name="sd.protocollo.protocollo"
    )

    protocollo_restore_mail_id = fields.Many2one(
        string="Original mail", comodel_name="mail.mail"
    )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            is_direction_in = (
                True if ("direction" in vals and vals["direction"] == "in") else False
            )
            is_pec = True if ("pec" in vals and vals["pec"]) else False
            is_posta_semplice = (
                True
                if (not ("pec_type" in vals) or vals["pec_type"] == False)
                else False
            )
            is_posta_certificata = (
                True
                if (
                    "pec_type" in vals
                    and vals["pec_type"] in ["posta-certificata", "errore"]
                )
                else False
            )
    ...
    def protocollo_crea_da_mail(
        ...
        mezzo_trasmissione_id = self.with_context(
            company_id=company_id
        )._get_mezzo_trasmissione_id()
        if not mezzo_trasmissione_id:
            raise ValidationError("Mezzo di trasmissione non trovato!")
        archivio_id = protocollo_obj.with_context(
            company_id=company_id
        )._default_archivio_id()
        if not archivio_id:
            raise ValidationError("Archivio corrente non trovato!")
        # recupero dei vals per la creazione del protocollo
        vals = self._get_protocollo_vals(
            vals, folder_id, mezzo_trasmissione_id, archivio_id
        )
        allegato_vals_list = []
        allegato_vals = {
        ...
        if self.original_eml and documento_tipologia == "eml":
            ...
            postacert_file = [
                x for x in self.original_attachment_ids if x.name == "postacert.eml"
            ]
            if postacert_file:
        ...
        body_pdf_content_encode = (
            base64.b64encode(body_pdf_content) if body_pdf_content else ""
        )
        ...
        for attachment in self.attachment_ids:
            ...
            if (
                documento_tipologia == "allegato"
                and attachment.id == documento_attachment_id
            ):
    ...
    def _get_body_pdf_content(self, company_id):
        report_mail_template = self.env.ref(
            "sd_protocollo_pec.report_sd_protocollo_pec_mail_mail_qweb"
        )
        pdf_mail_mail = (
            report_mail_template.sudo()
    ...
    def _get_utc_to_local_date(self):
        ...
        timezone = (
            self.env["ir.config_parameter"].sudo().get_param("sd_protocollo.timezone")
        )
        data_ricezione = utilities_obj.utc_to_local(self.date, timezone).strftime(
            "%d-%m-%Y %H:%M:%S"
        )
    ...
    def get_call_wizard_condition(self, call_action=False):
        ...
        wizard_obj = self.env[
            "sd.protocollo.wizard.protocollo.crea.da.mail"
        ].with_context(mail_id=self.id)
    ...
    def _get_mezzo_trasmissione_id(self):
        ...
        mezzo_trasmissione = self.env["sd.protocollo.mezzo.trasmissione"].search(
            domain, limit=1
        )
    ...
    def _get_protocollo_vals(self, vals, folder_id, mezzo_trasmissione_id, archivio_id):
        ...
        vals["data_ricezione"] = (
            self.server_received_datetime
            if self.server_received_datetime
            else self.date
        )
    ...
    def _get_mittente_vals(self):
        ...
        if re.findall('^"Per conto di: \S+@\S+"\\n* <[^>]+>', self.email_from):
            ...
            if results:
                mittente_email = (
                    results[0].replace('"', "").replace("Per conto di: ", "")
                )
                mittente_name = ""
            ...
            mittente_name = (
                self.email_from.replace(results[0], "").replace('"', "").strip()
            )
        ...
        digital_domicile = self.env["res.partner.digital.domicile"].search(
            domain, limit=1
        )
        if digital_domicile:
            # se esite un domicilio digitale con lo stesso indirizzo si copiano i valori della AOO per creare il contatto
            mittente = self.env["sd.dms.contact"].get_values_partner_contact(
                digital_domicile.aoo_id, partner_values=False
            )
        ...
        if email_address:
            ...
            mittente = self.env["sd.dms.contact"].get_values_partner_contact(
                email_address.partner_id, partner_values=False
            )
            mittente["typology"] = "sender"
            return mittente
    ...
    def _set_joins(self, query, alias_dict, operation="read"):
        ...
        protocollo_alias = query.left_join(
            "mail_mail", "protocollo_id", "sd_protocollo_protocollo", "id", "spp"
        )
        documento_alias = query.left_join(
            protocollo_alias, "id", "sd_dms_document", "protocollo_id", "sdd"
        )
    ...
    def _get_conditions(self, alias_dict, operation):
        ...
        conditions.append(
            self._get_permission_protocollo_mail_client_account_conditions(
                alias_dict, operation
            )
        )
        return conditions
    ...
    @api.model
    def _get_permission_protocollo_mail_client_account_conditions(
        self, alias_dict, operation
    ):
