...
# regex_field_agg, VALID_AGGREGATE_FUNCTIONS
...
class Base(models.AbstractModel):
    ...
    def _search_panel_domain_image(
        ...
        field = self._fields[field_name]
        if field.type != "many2many":
            return super(Base, self)._search_panel_domain_image(
    ...
    def read_group_m2m(
        ...
        result = self._read_group_raw_m2m(
            domain,
            fields,
            groupby,
            offset=offset,
    ...
    def _read_group_raw_m2m(
        ...
        for field in fields:
            query.join(
                self._table,
                "id",
                self._fields[field].relation,
                self._fields[field].column1,
                table_link_suffix,
            )
        # END ##########################################################################################################
        groupby = [groupby] if isinstance(groupby, str) else list(OrderedSet(groupby))
        ...
        if self.env.context.get("fill_temporal") and data:
            data = self._read_group_fill_temporal(
                data, groupby, aggregated_fields, annotated_groupbys
            )
    ...
    def _read_group_resolve_many2many_fields(self, data, fields):
        ...
        for field in many2manyfields:
            ...
            data_dict = dict(m2m_records.sudo().name_get())
            for d in data:
