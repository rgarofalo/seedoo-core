...
class Security(models.AbstractModel):
    ...
    inherit_acl = fields.Boolean("Inherit Acl", default=True)
    ...
    def _search(
        ...
        if self.skip_security():
            return super(Security, self)._search(args, offset, limit, order)
       
        # Ottieni il modello con i diritti di accesso dell'utente
    ...
    def create(self, vals):
        ...
        result.set_old_owner_acl(
            vals.get("owner_id", False), self.env.uid, self.env.user.name
        )
    ...
    def write(self, vals):
        ...
        if parent_field_value and self._parent_model:
            ...
            for record in self:
                if (
                    not record[self._parent_field]
                    or record[self._parent_field].id != parent_field_value
                ):
                    parent_model_obj.browse(parent_field_value)._check_access("create")
        elif (
            self._parent_field in vals and not parent_field_value and self._parent_model
        ):
            # le acl non possono essere ereditate se il record non ha padre, quindi si imposta inherit_acl a False
            vals["inherit_acl"] = False
        ...
        dict_old_acl_ids = {}
        for record in self:
            dict_old_acl_ids[record.id] = set(
                record.acl_ids.ids + record.system_acl_ids.ids
            )
    ...
    def flush(self, fnames=None, records=None):
        return super(Security, self.with_context(skip_security=True)).flush(
            fnames, records
        )
    ...
    def update_inherit_acl(
        ...
        self.ensure_one()
        if (
            not update_record_inherit_acls
            and not add_children_inherit_acl_ids
            and not del_children_inherit_acl_ids
        ):
            return
        ...
        if add_children_inherit_acl_ids or del_children_inherit_acl_ids:
            self._update_inherit_children_acl(
                self.id, add_children_inherit_acl_ids, del_children_inherit_acl_ids
            )
    ...
    def _get_parent_acl_ids(self, parent_id):
        ...
        """.format(
            ...
            parent_field=self.env[
                self._get_security_inherit_model_acl()
            ].get_acl_field(),
            parent_id=parent_id,
        )
        ...
        parent_acl_table_rel = (
            parent_obj._get_security_inherit_table_acl() + "_inherit_rel"
        )
        sql_query = """
            SELECT acl_id
            FROM {parent_acl_table_rel}
        ...
        """.format(
            parent_acl_table_rel=parent_acl_table_rel,
            parent_field=self.env[
                parent_obj._get_security_inherit_model_acl()
            ].get_acl_field(),
            parent_id=parent_id,
        )
        self.env.cr.execute(sql_query)
    ...
    def _insert_parent_acl_ids(self, record_id, parent_acl_ids):
        ...
        self.env.cr.execute(sql_query)
        _logger.debug(
            "--- insert parent_acl_ids %s in %s seconds ---"
            % (len(parent_acl_ids), time.time() - start_time)
        )
    ...
    @api.model
    def _update_inherit_children_acl(
        self, record_id, add_children_inherit_acl_ids, del_children_inherit_acl_ids
    ):
        ...
        for children_model in children_model_list:
            ...
            """.format(
                ...
                same_model_inherit_children_ids=",".join(
                    map(str, same_model_inherit_children_ids)
                ),
            )
            self.env.cr.execute(sql_query)
            ...
            for add_children_inherit_acl_id in add_children_inherit_acl_ids:
                self._insert_children_acl_id(
                    inherit_children_ids, children_model, add_children_inherit_acl_id
                )
            ...
            for del_children_inherit_acl_id in del_children_inherit_acl_ids:
                self._delete_children_acl_id(
                    inherit_children_ids, children_model, del_children_inherit_acl_id
                )
        ...
        for add_children_inherit_acl_id in add_children_inherit_acl_ids:
            self._insert_children_acl_id(
                same_model_inherit_children_ids,
                self._get_security_model(),
                add_children_inherit_acl_id,
            )
        # si elimina una acl per volta a tutti i figli che ereditano le acl dal record corrente
        for del_children_inherit_acl_id in del_children_inherit_acl_ids:
            self._delete_children_acl_id(
    ...
    def _insert_children_acl_id(self, children_ids, children_model, acl_id):
        ...
        children_acl_inherit_table_rel = (
            children_obj._get_security_table_acl() + "_inherit_rel"
        )
        ...
        """.format(
            ...
            children_field=self.env[
                children_obj._get_security_model_acl()
            ].get_acl_field(),
            values=",".join(inherit_children_acl_list),
        )
        self.env.cr.execute(sql_query)
        _logger.debug(
            "--- insert children_acl_id %s in %s seconds ---"
            % (len(children_ids), time.time() - start_time)
        )
    ...
    def _delete_children_acl_id(self, children_ids, children_model, acl_id):
        ...
        self.env.cr.execute(sql_query)
        _logger.debug(
            "--- delete children_acl_id %s in %s seconds ---"
            % (len(children_ids), time.time() - start_time)
        )
    ...
    def _set_acl_joins(self, query, alias_dict, join_prefix, inherit):
        ...
        table_acl_rel = "%s_acl_%srel" % (
            self._get_security_table(),
            "inherit_" if inherit else "",
        )
        ...
        table_acl_alias = query.left_join(
            table_acl_rel_alias, "acl_id", table_acl, "id", "%s_acl" % join_prefix
        )
        alias_dict[self._get_acl_rel_alias_key(join_prefix, inherit)] = (
            table_acl_rel_alias
        )
    ...
    def _get_author_conditions(self, alias_dict):
        return """{table}.owner_id = {uid}""".format(
            table=alias_dict.get(
                self._get_security_table(), self._get_security_table()
            ),
            uid=self.env.uid,
        )
    ...
    def skip_security(self):
        return (
            ...
            or self._context.get("skip_security", False)
        )
