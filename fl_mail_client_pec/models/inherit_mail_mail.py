...
class MailMail(models.Model):
    ...
    error = fields.Boolean(string="Reception Delivery Error", default=False)
    ...
    cert_datetime = fields.Datetime(string="Certified Datetime", readonly=True)

    recipient_addr = fields.Char(string="Recipient Address", readonly=True)

    pec_msg_id = fields.Char(string="PEC Message Id", readonly=True)

    pec_mail_parent_id = fields.Many2one(
        "mail.mail", string="Parent PEC Mail", readonly=True
    )

    pec_mail_child_ids = fields.One2many(
        "mail.mail", "pec_mail_parent_id", string="Related Notifications", readonly=True
    )

    resized_subject = fields.Char(
        string="Resized Subject", compute="_compute_resized_subject"
    )

    pec_mail_child_count = fields.Integer(
        string="Related Notifications", compute="_compute_pec_mail_child_count"
    )
    ...
    def _compute_resized_subject(self):
        for rec in self:
            ...
            if len(rec.subject) > 80:
                ...
                resized_subject = (
                    str_subject + " ..." if len(list) != len(subject) else str_subject
                )
            rec.resized_subject = resized_subject

    def _compute_pec_mail_child_count(self):
        for rec in self:
            rec.pec_mail_child_count = self.search_count(
