...
class ResUsers(models.Model):
    ...
    def __init__(self, pool, cr, uid):
        init_res = super().__init__(pool, cr, uid)
        type(self).SELF_WRITEABLE_FIELDS = self.SELF_WRITEABLE_FIELDS + [
            "default_folder_id"
        ]
    ...
    def _request_notify_message(self, type, title, message, sticky=False):
        ...
        message = {"title": title, "message": message, "sticky": sticky}
        if type == "success":
    ...
    def _update_folder_acl(
        ...
        for record in self:
            ...
            if (
                record.with_user(record).user_has_groups(group_xml_id)
                and not folder_acl
            ):
                folder.write(
                    {
                ...
                not record.with_user(record).user_has_groups(group_xml_id)
                and folder_acl
            ):
                folder_acl.unlink()
